...
- **弹窗**：深色主题下为 Win11 Mica 系统材质（浅色主题用纯色——实测该窗口形态 Mica 色调恒深，浅色配上会破坏对比度，且浅色 Mica 与纯色观感几乎一致；设置页可强制纯色），唤出时 200ms 淡入上滑动效（跟随系统动画开关）；自动预填剪贴板（可关，无则示例文字），输入实时预览（150ms 防抖，预览只转换前 64 字素）；窗口出现在鼠标附近（可改主屏居中），点击别处自动隐藏。
- **操作**：`Enter` 复制选中样式的**全文**转换（选中行 ⏎ 闪烁反馈）并隐藏；`Esc` 隐藏；`Ctrl+D` 收藏（⭐，插件同步可见）；`Ctrl+R` 随机换一个；`↑↓` 浏览；输入行右侧「全部 ▾」下拉按六大类筛选，⭐/🕘 图标直达收藏/最近。
- **设置**（托盘右键 → 设置，全部即时生效）：主题（浅色/深色/跟随系统，系统切换明暗自动跟随）、背景材质（Mica（深色主题）/ 纯色）、强调色（色板/自定义/**跟随系统强调色**，系统改色后自动跟随）、预览字号、语言（中/英）、唤出快捷键录制、开机自启动、唤出时预填选中文字/剪贴板、复制后收起、弹窗位置。
