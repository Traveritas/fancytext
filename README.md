...
- **弹窗**：Win11 Mica 系统材质，明暗主题自动跟随（设置页可切纯色；实现要点：无边框窗口需先 `DwmExtendFrameIntoClientArea(-1)` 扩展框架区，Mica 才有落笔处），唤出时 200ms 淡入上滑动效（跟随系统动画开关）；自动预填剪贴板（可关，无则示例文字），输入实时预览（150ms 防抖，预览只转换前 64 字素）；窗口出现在鼠标附近（可改主屏居中），点击别处自动隐藏。
